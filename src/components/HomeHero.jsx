...
import cvFile from '../downloads/CV_ATS.pdf';

export default function HomeHero({ isBright, scrollProgress, heroRef, onNavigate }) {
    const titles = ['Full-Stack Developer', 'Mobile Developer', 'Data Analyst'];
    ...
    return (
        <div ref={heroRef} className="min-h-[200vh]">
            <section className="relative h-screen flex items-center justify-center overflow-hidden sticky top-0">
                ...
                <div className={`absolute inset-0 ${isBright ? 'bg-gradient-to-b from-horizon-bg/20 via-horizon-card/60 to-horizon-bg' : 'bg-gradient-to-b from-midnight-bg/20 via-midnight-card/50 to-midnight-bg'} z-5 pointer-events-none`}></div>
                ...
                <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8">
                    <div className="max-w-7xl mx-auto grid md:grid-cols-2 gap-12 items-center">
                        ...
                        <div className="text-center md:text-left space-y-6 order-1 md:order-2">
                            ...
                            <div className="min-h-[3rem] sm:min-h-[3.5rem] lg:min-h-[3rem]">
                                <p className={`text-2xl sm:text-3xl lg:text-4xl ${isBright ? 'text-horizon-text' : 'text-midnight-text'} mb-8 font-semibold`}>
                                    {displayText}
                                    <span className={`${isBright ? 'text-horizon-accent' : 'text-midnight-accent'} animate-pulse`}>|</span>
                                </p>
                            </div>
                            ...
                            <div className="flex flex-wrap gap-6 justify-center md:justify-start items-center">
                                <a
                                    href="https://github.com/etherealesz"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`${isBright ? 'text-horizon-text hover:text-horizon-accent' : 'text-midnight-text hover:text-midnight-accent'} transition-all duration-300 transform hover:scale-110`}
                                    title="GitHub"
                                >
                                ...
                                <a
                                    href="https://instagram.com/stefanusenardo"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`${isBright ? 'text-horizon-text hover:text-horizon-accent' : 'text-midnight-text hover:text-midnight-accent'} transition-all duration-300 transform hover:scale-110`}
                                    title="Instagram"
                                ...
                                <a
                                    href="https://linkedin.com/in/enardo-dev"
                                    target="_blank"
                                    rel="noopener noreferrer"
                                    className={`${isBright ? 'text-horizon-text hover:text-horizon-accent' : 'text-midnight-text hover:text-midnight-accent'} transition-all duration-300 transform hover:scale-110`}
                                    title="LinkedIn"
